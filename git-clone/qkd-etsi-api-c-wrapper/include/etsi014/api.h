...
typedef struct qkd_cert_config {
    const char *cert_path;    /* Path to client certificate */
    const char *key_path;     /* Path to client private key */
    const char *ca_cert_path; /* Path to CA certificate */
    const char *sae_id;       /* SAE ID for X-SAE-ID header */
} qkd_cert_config_t;
