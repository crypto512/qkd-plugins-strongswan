 ...
 * Gets certificate configuration from global config set by plugin
 */
int init_cert_config(int role, etsi014_cert_config_t *config) {
    (void)role;  /* role parameter unused - same config for both sides */

    qkd_cert_config_t global_config;
    if (QKD_014_GET_CERT_CONFIG(&global_config) != 0) {
        QKD_DBG_ERR("Certificate configuration not set - call QKD_014_SET_CERT_CONFIG first");
        return QKD_STATUS_BAD_REQUEST;
    }

    config->cert_path = global_config.cert_path;
    config->key_path  = global_config.key_path;
    config->ca_cert_path = global_config.ca_cert_path;

    QKD_DBG_INFO("Certificate configuration retrieved:");
...
static char *handle_request_https(const char *url, const char *post_data, long *http_code,
    ...
    if(curl) {
        ...
        /* Add X-SAE-ID header from global config for KME authentication */
        qkd_cert_config_t global_cfg;
        if (QKD_014_GET_CERT_CONFIG(&global_cfg) == 0 && global_cfg.sae_id) {
            char sae_header[256];
            snprintf(sae_header, sizeof(sae_header), "X-SAE-ID: %s", global_cfg.sae_id);
            headers = curl_slist_append(headers, sae_header);
            QKD_DBG_INFO("Added X-SAE-ID header: %s", global_cfg.sae_id);
        }
...
static char *build_post_data(qkd_key_ids_t *key_ids) {
    if (!key_ids || key_ids->key_ID_count <= 0) {
        return NULL;
    }

    // Calculate required buffer size: each entry ~60 chars + overhead
    size_t buffer_size = 32 + (key_ids->key_ID_count * 80);
    char *post_data = calloc(buffer_size, sizeof(char));
    if (!post_data) {
        return NULL;
    }
    ...
    size_t offset = 0;
    offset += snprintf(post_data + offset, buffer_size - offset, "{\"key_IDs\":[");

    for (int i = 0; i < key_ids->key_ID_count; ++i) {
        if (offset >= buffer_size - 64) {
            QKD_DBG_ERR("build_post_data: buffer too small");
            free(post_data);
            return NULL;
        }
        offset += snprintf(post_data + offset, buffer_size - offset,
                          ...
                          (i < key_ids->key_ID_count - 1) ? "," : "");
    }
    snprintf(post_data + offset, buffer_size - offset, "]}");
    return post_data;
}

/* Timeout configuration (can be overridden via environment) */
...
static char *handle_request_https(const char *url, const char *post_data, long *http_code,
    ...
    if(curl) {
        ...
        long connect_timeout = DEFAULT_CONNECT_TIMEOUT_SEC;
        long transfer_timeout = DEFAULT_TRANSFER_TIMEOUT_SEC;
        const char *env_connect = getenv("QKD_CONNECT_TIMEOUT");
        const char *env_transfer = getenv("QKD_TRANSFER_TIMEOUT");
        if (env_connect) connect_timeout = atol(env_connect);
        if (env_transfer) transfer_timeout = atol(env_transfer);
...
static uint32_t handle_http_response(const char *response, long http_code, 
    ...
    if (response && http_code < 400) {
        ...
        QKD_DBG_ERR("[HTTP_RESPONSE_HANDLER] - HTTP request failed with code %ld.", http_code);
        if (response) {
            free((void *)response);
        }
        /* Map HTTP codes to specific QKD status codes for better error handling */
        switch (http_code) {
            case 200:
                return QKD_STATUS_OK;
            case 400:
            ...
            case 403:
                return QKD_STATUS_FORBIDDEN;
            case 404:
                return QKD_STATUS_NOT_FOUND;
            case 409:
                return QKD_STATUS_CONFLICT;
            case 503:
                return QKD_STATUS_SERVER_ERROR;
            default:
                /* Generic fallback: 4xx -> client error, 5xx -> server error */
...
static uint32_t get_key(const char *kme_hostname,
    ...
    int size_keys = (request && request->size > 0) ? request->size : DEFAULT_KEY_SIZE;
